...
<CrossCourseRef
  ...
  label="Spark transformWithState: альтернативный stateful pattern"
  preview="Как Spark Structured Streaming держит per-key state с TTL и timers — параллельное решение для другого класса задач (windowed aggregations, sessions) поверх Kafka source."
/>
